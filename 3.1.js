...
const getCoordinateIncrementFunc = (direction) => {
  switch (direction) {
    case 'R':
      return ([startX, startY], increment) => [startX + increment, startY];
    case 'L':
      return ([startX, startY], increment) => [startX - increment, startY];
    case 'U':
      return ([startX, startY], increment) => [startX, startY + increment];
    case 'U':
    default:
      return ([startX, startY], increment) => [startX, startY - increment];
  }
...
const getAllCoordinates = wire => {
  ...
  for (let i = 0, l = wire.length; i < l; i++) {
    ...
    const [lastCoords] = coordinates.slice(-1);
